...
public class OpenAiService {
    ...
    public void createCompletion(CompletionRequest request, Consumer<CompletionResult> resultConsumer) {
        execute(api.createCompletion(request), resultConsumer);
    }
    ...
    public void createChatCompletion(ChatCompletionRequest request, Consumer<ChatCompletionResult> resultConsumer) {
        execute(api.createChatCompletion(request), resultConsumer);
    }
     ...
     */
    public static <T> T execute(Call<T> apiCall) {
        try {
     ...
     * Calls the Open AI api, returns the response, and parses error messages if the request fails
     */
    public static <T> void execute(Call<T> apiCall, Consumer<T> resultConsumer) {
        apiCall.enqueue(new Callback<T>() {
            @Override
            public void onResponse(Call<T> call, Response<T> response) {
                if (response.isSuccessful()) {
                    resultConsumer.accept(response.body());
                } else {
                    OpenAiError error;
                    try {
                        error = errorMapper.readValue(response.errorBody().string(), OpenAiError.class);
                    } catch (IOException e) {
                        e.printStackTrace();
